...
export const UserSummaryCard: React.FC<Props> = ({
  ...
  const missing = Math.max(0, totalLangs - uniqueCapturedCount);

  return (
    <section
      className="max-w-6xl mx-auto mb-6 rounded-3xl border border-white/10 bg-white/5
    ...
    >
      <div className="flex items-center gap-4 sm:gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded-full text-xs font-semibold border
              ...
              LangDex: {uniqueCapturedCount}/{totalLangs} {missing > 0 ? `(faltam ${missing})` : ""}
            </span>
          </div>
        ...
        {/* CTA */}
        {uniqueCapturedCount === 0 && onStartJourney ? (
