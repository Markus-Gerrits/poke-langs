...
import type { Scene, Step, SayStep, ChoiceStep, CallStep } from "./types";
import DialogBox from "../components/DiologBox";

type Props = {
    scene: Scene;
    api?: Record<string, (args?: any, helpers?: { getFlag: (k:string)=>any; setFlag:(k:string,v:any)=>void; }) => any | Promise<any>>;
    onEnd?: () => void;
    className?: string;
};
...
function buildLabelIndex(steps: Step[]) {
    ...
    steps.forEach((s, i) => {
        if ("label" in s && s.label) map.set(s.label, i);
    });
    return map;
}
...
const CutscenePlayer: React.FC<Props> = ({ scene, api, onEnd, className }) => {
    ...
    if (step.t === "set") {
        setFlag(step.flag, (step as any).value);
        nextStep(step.next);
        return null;
    }
    if (step.t === "end") {
    ...
    if (step.t === "call") {
        ...
        (async () => {
            if (!fn) {
            console.warn(`Cutscene: função não encontrada: ${s.fn}`);
            if (mounted) nextStep(s.next);
            return;
            }
            try {
            setBusy(true);
            ...
            await fn(s.args, { getFlag, setFlag });
            } finally {
            if (mounted) {
                setBusy(false);
                nextStep(s.next);
        ...
        // mostra o último balão (se existir) “ocupado” ou um loading simples
        return (
        <div className={className}>
            <div className="absolute left-1/2 -translate-x-1/2 bottom-6 w-full max-w-5xl px-4">
            <DialogBox
                speaker="..."
                text={busy ? "Processando..." : "Ok!"}
    ...
    if (icon.includes("/") || icon.endsWith(".png") || icon.endsWith(".svg") || icon.endsWith(".jpg"))
        return icon;
        ...
        <div className={className}>
            <div className="absolute left-1/2 -translate-x-1/2 bottom-[clamp(12px,4vh,40px)] w-full max-w-5xl px-4">
                ...
                <>
                    ...
                    const hasIcons = s.options.some(o => !!o.icon);
                    ...
                    if (useCards) {
                        ...
                        <div className={`mt-3 grid gap-4 ${cols === 2 ? "sm:grid-cols-2" : "sm:grid-cols-3"}`}>
                            ...
                            <ChoiceCard
                                ...
                                iconSrc={resolveIconSrc(opt.icon)}
  ...
  return (
    <button
      ...
      className="
        ...
        focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 cursor-pointer
      "
      aria-label={label}
    >
